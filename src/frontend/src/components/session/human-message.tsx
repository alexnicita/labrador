...
export function HumanMessage({ actor, message, onComment, onReact }: HumanMessageProps) {
  ...
  return (
    <article
      className={cn(
        "flex items-start gap-4 rounded-[14px] border px-2 py-3 transition-colors sm:px-3",
        message.selected
    ...
    >
      ...
      <div className="min-w-0 pt-1">
        ...
        <p className="mt-2 text-[14px] leading-6 text-[#141820]">{message.body}</p>
        {showActions ? (
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              {...(onReact ? { onClick: () => onReact("thumbs_up") } : {})}
            >
